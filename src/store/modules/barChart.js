...
const state = {
   queries: {
     //General
       'numberr': {
           result: null,
           sql: ` select 
           sum(FEE/1e9) as transaction_fee, 
           count(*) as number ,
           avg(FEE/1e9) as agfees,
           count(distinct SIGNERS[0]) as daily_active_user, 
           count(case when SUCCEEDED='TRUE' then 1 else null end)*100/number as success_rate, 
 ...
 'projects': {
  ...
  select NUMBER,PROGRAM_ID,ADDRESS_NAME
  from 
  t1 a inner join solana.core.dim_labels b
...
'stakee': {
  ...
  sql: `select 
  date_trunc('week',BLOCK_TIMESTAMP) as DATE,
  sum (case when ACTION in ('withdraw_dao_stake','withdraw','withdraw_stake','withdraw_dao') then AMOUNT/1e9 else null end ) as unstake_volume,
  sum (case when ACTION in ('deposit_dao_with_referrer','deposit_dao_stake','deposit_dao','deposit','deposit_stake') then AMOUNT/1e9 else null end ) as stake_volume,
  sum (case when ACTION in ('claim') then AMOUNT/1e9 else null end ) as get_reward_volume, 
  count (case when ACTION in ('withdraw_dao_stake','withdraw','withdraw_stake','withdraw_dao') then 1 else null end) as unstake_number,
  count (case when ACTION in ('deposit_dao_with_referrer','deposit_dao_stake','deposit_dao','deposit','deposit_stake') then 1 else null end ) as stake_number,
  count (case when ACTION in ('claim') then 1 else null end ) as get_reward_number,
  ...
  sum(get_reward_volume) over (order by date) as cumulative_get_reward_volume,
  cumulative_stake_volume-(cumulative_unstake_volume+cumulative_get_reward_volume) as solana_flow
...
'stake1': {
  ...
  sql: `with ggg as
  (select 
  sum(case when ACTION in ('withdraw_dao_stake','withdraw','withdraw_stake','withdraw_dao') then AMOUNT/1e9 else null end) as staking_volume, 
  sum(case when ACTION in ('deposit_dao_with_referrer','deposit_dao_stake','deposit_dao','deposit','deposit_stake') then AMOUNT/1e9 else null end ) as unstaking_volume,   
  ADDRESS
    from 
...
'bridge33': {
  ...
  select 
    ...
    sum(txn_count) over (order by date asc) as comulative_txn_count,
    sum(users_count) over (order by date asc) as comulative_users_count,
    sum(volume_usd) over (order by date asc) as comulative_volume_usd,
    sum(volume_usd_avg) over (order by date asc) as comulative_volume_usd_avg
  from ethereum.core.fact_transactions a
  join ethereum.core.ez_token_transfers b using (tx_hash)
  join prices
  on trunc(block_timestamp, 'week') = prices.day
