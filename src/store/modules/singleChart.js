...
// sigles charst season 0 
    'users': {
      result: null,
      sql: `select count(distinct(SIGNERS[0]))
      from 
    ...
    'NFTmint': {
      ...
      sql: `Select 
      count(*) as number , 
      count(distinct PROGRAM_ID) as project_number, 
      count(distinct PURCHASER) as minter_number, 
    ...
    'NFTseals': {
      result: null,
      sql: `select 
      count(distinct MARKETPLACE) as marketplace_number,
      count(distinct PURCHASER) as user_number,
      count(distinct MINT) as total_NFT,
      avg(SALES_AMOUNT) as average_seals, 
      sum(SALES_AMOUNT)/marketplace_number as sum_per_market
    ...
    'stake': {
      ...
      stake_volume-unstake_volume as total_solana, 
      count(distinct STAKE_POOL) as staking_pool_number, 
